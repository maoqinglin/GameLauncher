...
public class HListView extends AbsHListView {
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (heightMode == MeasureSpec.UNSPECIFIED) {
			...
			int[] result = measureWithLargeChildren(heightMeasureSpec, mMeasureWithChild, mMeasureWithChild, widthSize,
					heightSize, -1);
			heightSize = result[1];
	...
	final int[] measureWithLargeChildren(int heightMeasureSpec, int startPosition, int endPosition, final int maxWidth,
	...
	private void setupChild(View child, int position, int x, boolean flowDown, int childrenTop, boolean selected,
		...
		if (needToMeasure) {
			int childHeightSpec = ViewGroup.getChildMeasureSpec(mHeightMeasureSpec, mListPadding.top
					+ mListPadding.bottom, p.height);
			int lpWidth = p.width;
			int childWidthSpec;
			if (lpWidth > 0) {
				childWidthSpec = MeasureSpec.makeMeasureSpec(lpWidth, MeasureSpec.EXACTLY);
	...
	private boolean commonKey(int keyCode, int count, KeyEvent event) {
		...
		if (action != KeyEvent.ACTION_UP) {
			...
			case KeyEvent.KEYCODE_DPAD_LEFT:
				if (mLastKeyStartTime > 0 && mLastKeyCode == KeyEvent.KEYCODE_DPAD_LEFT) {
					if (System.currentTimeMillis() - mLastKeyStartTime < KEY_INTERNAL) {
						return true;
					}
				}
				...
				mLastKeyStartTime = System.currentTimeMillis();
				mLastKeyCode = event.getKeyCode();
				break;

			case KeyEvent.KEYCODE_DPAD_RIGHT:
				// modify by linmaoqing 2014-11-24解决长按右键焦点丢失问题
				if (mLastKeyStartTime > 0 && mLastKeyCode == KeyEvent.KEYCODE_DPAD_RIGHT) {
					if (System.currentTimeMillis() - mLastKeyStartTime < KEY_INTERNAL) {
						return true;
					}
				}
	...
	private boolean arrowScrollImpl(int direction) {
		...
		if (amountToScroll > 0) {
			// modify by linmaoqing 2014-11-24 解决item过长导致滚动异常的问题
	...
	private void measureItem(View child) {
		...
		int childHeightSpec = ViewGroup.getChildMeasureSpec(mHeightMeasureSpec, mListPadding.top + mListPadding.bottom,
				p.height);
		int lpWidth = p.width;
		int childWidthSpec;
		if (lpWidth > 0) {
	...
	private int getCenterOfListView() {
		return getWidth() / 2;
	}
	...
	private int getScrollDistance(View nextVisibleView) {
		return Math.abs(getCenterOfView(nextVisibleView) - getCenterOfListView());
	}
	...
	public View findViewByPosition(int position) {
		...
		View v = getChildAt(position - firstPosition);
		return v;
	}

	@Override
	public int getChildDrawingOrder(int childCount, int position) {
		...
		if (position < selectedIndex) {
			...
			return childCount - 1 - position + selectedIndex;
		} else {
			return position;
		}
	}
	...
	protected void onFocusChanged(boolean gainFocus, int direction, Rect previouslyFocusedRect) {
		...
		if (adapter != null && gainFocus && previouslyFocusedRect != null) {
			...
			for (int i = 0; i < childCount; i++) {
				...
				// by linmaoqing 从顶部菜单切换到listview时，会水平滚动导致焦点丢失
	...
	private OnKeyListener mExternalKeyListener = new OnKeyListener() {
