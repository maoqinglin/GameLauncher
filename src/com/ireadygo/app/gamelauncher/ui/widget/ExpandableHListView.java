...
public class ExpandableHListView extends HListView {
	...
	protected void dispatchDraw( Canvas canvas ) {
		...
		final Rect groupTitleRect= mIndicatorRect;
		...
		for( int i = 0, childFlPos = mFirstPosition - headerViewsCount; i < childCount; i++, childFlPos++ ) {
			...
			if( indicatorRect.top != indicatorRect.bottom ) {
				...
				if( indicator != null ) {
					...
					// Draw the indicator  modify by linmaoqing
//					indicator.setBounds( mTempRect );
//					indicator.draw(canvas);
					if (pos.groupMetadata != null) {
						int groupPos = pos.groupMetadata.gPos;
						if (groupPos != -1 && mAdapter instanceof HExpandableListAdapter) {
							Drawable groupTitleDrawable = ((HExpandableListAdapter) mAdapter).getGroupTitleBg(indicator,
									groupPos);
							
							if (groupTitleDrawable != null) {
								groupTitleDrawable.setBounds(mTempRect);
								groupTitleDrawable.draw(canvas);
							}
						}
					}
				}
