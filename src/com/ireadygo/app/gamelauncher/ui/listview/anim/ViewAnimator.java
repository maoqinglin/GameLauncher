...
/**
 * A class which decides whether given Views should be animated based on their
 * position: each View should only be animated once. It also calculates proper
 * animation delays for the views.
 */
public class ViewAnimator {
	...
	public static final int DEFAULT_ANIMATION_DELAY_MILLIS = Config.Animator.DELAY_SHORT;
	/**
	 * The default duration in millis of the animations.
	 */
	// private static final int DEFAULT_ANIMATION_DURATION_MILLIS = 300;
	public static final int DEFAULT_ANIMATION_DURATION_MILLIS = Config.Animator.DURATION_SHORT;
	...
	/**
	 * The start timestamp of the first animation, as returned by
	...
	/**
	 * Whether animation is enabled. When this is set to false, no animation is
	 * applied to the views.
	 */
	private boolean mShouldAnimate = true;

	/**
	 * Creates a new ViewAnimator, using the given
	 * {@link com.nhaarman.listviewanimations.util.ListViewWrapper}.
	 * 
	 * @param listViewWrapper
	 *            the {@code ListViewWrapper} which wraps the implementation of
	 *            the ListView used.
	 */
	 ...
	 * will animate as well. Will also call {@link #enableAnimations()}.
	 * 
	 * @param position
	 *            the position.
	 */
	...
	/**
	 * Sets the value of the last animated position. Views with positions
	 * smaller than or equal to given value will not be animated.
	 */
	 ...
	 * Defaults to {@value #INITIAL_DELAY_MILLIS}.
	 * 
	 * @param delayMillis
	...
	/**
	 * Sets the delay in milliseconds before an animation of a view should
	 * start. Defaults to {@value #DEFAULT_ANIMATION_DELAY_MILLIS}.
	 * 
	 * @param delayMillis
	 *            the time in milliseconds.
	 */
	...
	/**
	 * Sets the duration of the animation in milliseconds. Defaults to
	 * {@value #DEFAULT_ANIMATION_DURATION_MILLIS}.
	 * 
	...
	/**
	 * Disables animating the Views. Enable them again using
	 * {@link #enableAnimations()}.
	 */
	...
	private void animateView(final int position, final View view, final Animator[] animators) {
		...
		animatorSet.play(set).after(calculateAnimationDelay(position));
		animatorSet.start();
	...
	private int calculateAnimationDelay(final int position) {
		...
		if (numberOfItemsOnScreen + 1 < numberOfAnimatedItems) {
			...
			delay = Math
					.max(0,
							(int) (-SystemClock.uptimeMillis() + mAnimationStartMillis + mInitialDelayMillis + delaySinceStart));
		}
		return delay;
	}
