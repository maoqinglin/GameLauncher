...
public abstract class AnimationAdapter extends BaseAdapterDecorator {
	private static final int WHAT_FIRST_LOADED_FINISH = 1;
	private Handler mHandler = new Handler(){
		public void handleMessage(Message msg) {
			switch (msg.what) {
			case WHAT_FIRST_LOADED_FINISH:
    ...
    public final View getView(final int position,  final View convertView,  final ViewGroup parent) {
        ...
        if (mIsRootAdapter && convertView == null) {
        	if(mHandler.hasMessages(WHAT_FIRST_LOADED_FINISH)){
        		mHandler.removeMessages(WHAT_FIRST_LOADED_FINISH);
        	}
        	animateViewIfNecessary(position, itemView, parent);
            mHandler.sendEmptyMessageDelayed(WHAT_FIRST_LOADED_FINISH, 500);
        }
        return itemView;
    }
