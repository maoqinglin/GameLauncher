...
public class LoadThemeUtil {
	...
	private static final String THEME_ICON_HIGH_LIGTH = THEME_ID + SEPARATOR + "much_icon_hightlight.png";
	...
	public static class ThemeApp {
		...
		public ThemeApp(String packageName, String className, String iconId, String themeId) {
			mPackageName = packageName;
			mClassName = className;
			mIconId = iconId;
			mThemeId = themeId;
		...
		public boolean equals(Object other) {
			...
			return mPackageName.equals(otherApp.mPackageName) && mClassName.equals(otherApp.mClassName);
		}
		...
		public String toString() {
			return "App [packageName=" + mPackageName + ", className=" + mClassName + ", iconid=" + mIconId
					+ ", themeid=" + mThemeId + "]";
		}
	}
	...
	public static ArrayList<ThemeApp> loadThemeInfos(Context context, String themeId) {
	...
	public static Bitmap getThemeIcon(Context context, String path) {
		AssetManager am = context.getAssets();
		try {
			//modify by linmaoqing 2014-11-20
			return BitmapFactory.decodeStream(am.open(path));
//			Bitmap source = BitmapFactory.decodeStream(am.open(path));
//			int appSize = context.getResources().getDimensionPixelSize(R.dimen.mygame_app_icon_size);
//			if(source != null){
//				source = ThumbnailUtils.extractThumbnail(source, appSize, appSize);
//			}
			
//			return PictureUtil.zoomImage(source, appSize, appSize);
		} catch (IOException e) {
			return null;
		}
	}
...
//		path = THEME_ICON_PATTERN_TRANSLUCENT;
		...
		if (bitmap == null) {
			bitmap = BitmapFactory.decodeResource(context.getResources(), R.drawable.much_app_icon_bg);
		}
		return bitmap;
	}

	public static Bitmap getIconHighlight(Context context) {
		String path = THEME_ICON_HIGH_LIGTH;
//		path = THEME_ICON_PATTERN_TRANSLUCENT;
		Bitmap bitmap = getThemeIcon(context, path);
		if (bitmap == null) {
			bitmap = BitmapFactory.decodeResource(context.getResources(), R.drawable.much_app_icon_bg);
	...
	public static Bitmap getIconMask(Context context) {
		...
		if (bitmap == null) {
			bitmap = BitmapFactory.decodeResource(context.getResources(), R.drawable.much_icon_mask);
		}
		return bitmap;
	}
	...
	static class SimpleSaxHandler extends DefaultHandler {
		...
		public void startElement(String uri, String localName, String qName, Attributes attributes) throws SAXException {
			if (APPLICATION.equals(localName)) {
				...
				apps.add(new ThemeApp(packageName, className, iconName, mThemeId));
			}
		}
