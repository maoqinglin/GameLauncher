...
public class UpdateManager implements LocalDataLoadCallback {
	...
	private static final int MSG_CHECK_UPGRADE_POLLING = 101;
	...
	private static final long MIN_UPGRADE_TIP_DELAY = 5 * 1000;//5 second
		...
		mThreadPool.execute(new Runnable() {
			...
			public void run() {
				try {
					...
					List<AppEntity> upgradeables = GameInfoHub.instance(mContext).obtainUpdatableApp(checkAppList);
					...
					for (final AppEntity app : upgradeables) {//修改应用的更新标志位
						AppEntity newApp = GameInfoHub.instance(mContext).obtainItemById(app.getAppId());
						mGameData.updateUpgradeAppData(newApp, app);
						...
						mHandler.postDelayed(new Runnable() {
							...
							@Override
							public void run() {
								mGameLauncherNotification.addUpgradeNotification(app);
							}
						}, MIN_UPGRADE_TIP_DELAY);
					}
					//是否通知满足最小通知间隔
...
//						mGameLauncherNotification.addUpgradeNotification(upgradeables.size());
	...
	private Handler mHandler = new Handler() {
		public void handleMessage(Message msg) {
			...
			case MSG_CHECK_UPGRADE:
				doCheckUpgradeApps();
				break;
			case MSG_CHECK_UPGRADE_POLLING:
				doCheckUpgradeApps();
				postMsg(MSG_CHECK_UPGRADE_POLLING, MIN_UPGRADE_CHECK_DELAY);
				break;
			default:
				break;
			}
	...
	public void loadSuccess() {
		postMsg(MSG_CHECK_UPGRADE_POLLING, 30 * 1000);
